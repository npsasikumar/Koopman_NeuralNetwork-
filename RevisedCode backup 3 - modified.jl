...
# ╔═╡ 803c9ad0-afcc-11f0-0f80-99aa3ad8931d
using Flux

# ╔═╡ 5f1f0a58-33e4-4463-8125-a87548a11cb7
using Flux: params

# ╔═╡ 5e888c05-b18b-4e6c-836e-6410063f5e24
using DifferentialEquations

# ╔═╡ 106ae79a-73a5-4f6e-a814-79995c3223fb
using Plots

# ╔═╡ 08b92ced-d035-4374-9ace-e9b4ec71060a
using LinearAlgebra

# ╔═╡ 563d803b-b59b-4297-b350-02ea817101a2
using Dates # For high-precision timing

# ╔═╡ 34c23f2d-f77d-4e81-befa-1024d2ba0c5a
using DataFrames

# ╔═╡ 1bf28515-a891-417f-bac8-f9afa5b8caf1
using ExcelFiles

# ╔═╡ dcdf5327-c35e-4393-af22-2deb40629d05
using XLSX

# ╔═╡ 5129d4eb-25bc-4918-80cd-618254ccafaa
using StatsBase

# ╔═╡ 6d7fa8b7-5d2d-4fdb-92c6-e15a6f70deee
using GlobalSensitivity, Statistics

# ╔═╡ 3e76acfa-f50d-4d08-9063-5f90a36b3c1c
using CairoMakie

# ╔═╡ dc201ed9-c564-4ac5-b568-15033bf2ab49
using QuasiMonteCarlo, StableRNGs

# ╔═╡ e598c5cf-94e2-4ea5-9e24-49857cf84399
start_time = now()

# ╔═╡ 66b0932a-70cc-4b9a-8b34-1deb7e4d21c7
# Defien the updated Rossler system
function rossler_with_control!(du, u, p, t)
	...
	du[4] = -ys - zs + k1*(xm - xs)
	du[5] = xs + a*ys + k2*(ym - ys)
	du[6] = b + zs*(xs - c) + k3*(zm - zs)
end

# ╔═╡ 82d0c550-f783-49b3-b10d-34d4b9c2371c
# Initial conditions and parametes
u0 = [1.0, 0.0, 1.5, 0.5, 0.5, 0.5] # Initial states [xm, ym, zm, xs, ys, zs]

# ╔═╡ 3892e18b-1353-47b0-8f5e-6fcb1fbccaf6
p = [0.2, 0.2, 9.0, -0.5, -0.5, -0.5]

# ╔═╡ 162823da-c9ff-42b6-9ab9-9f8028cd07de
tspan = (0.0, 1000.0)

# ╔═╡ 17dffadb-9923-4f46-9e93-a637c35dbec5
dt = 0.001

# ╔═╡ da31bda9-4f47-41df-b4f4-91a4afce46a3
prob = ODEProblem(rossler_with_control!, u0, tspan, p)

# ╔═╡ 3aa96097-2bc2-44b7-ada1-a31295bfa8fe
sol = solve(prob, Tsit5(), dt = dt, adaptive = false)

# ╔═╡ a6a86fcb-3f60-42b8-9956-503b4e7d27d6
size(sol)

# ╔═╡ e358517e-8951-4e33-857a-ab8ebe0f9a5e
# Extract master states [xm, ym, zm]
master_states = hcat([s[1:3] for s in sol.u]...)  # 3 × N matrix

# ╔═╡ 996aec21-7638-4eca-8012-b11225682cd6
function psi(x)
    xm, ym, zm = x
    return [xm, ym, zm, xm^2, ym^2, zm^2, ym*zm, xm*zm, zm*xm, 1.0]
end

# ╔═╡ be3e63d2-6117-479f-a581-cb951c8a55f2
X = hcat([psi(master_states[:, i]) for i in 1:(size(master_states, 2)-1)]...)

# ╔═╡ 999b0207-fbcf-4f84-98c1-7fea8745b33c
Y = hcat([psi(master_states[:, i+1]) for i in 1:(size(master_states, 2)-1)]...)

# ╔═╡ 98d3b39e-a1da-4c62-ac29-a01a89063e00
K = (Y) * pinv(X)

# ╔═╡ 27ba8770-0626-4b0d-ba79-6e97de06c145
ψ0 = psi(master_states[:, 1])

# ╔═╡ fad0cf00-b250-4f38-86dc-6c4e2d4c5fee
ψ_pred = K * ψ0

# ╔═╡ 3cfbcda0-d073-4978-b548-8a6d21ac4984
println("Koopman-predicted lifted state (first step): ", ψ_pred)

# ╔═╡ 48a65b30-3f12-4b8a-90d1-268e6c45cec7
println("Actual lifted next state: ", psi(master_states[:, 2]))

# ╔═╡ d9821f0a-b704-497b-aebc-1bd120a4cdc0
errors = [norm(K * psi(master_states[:, i]) - psi(master_states[:, i+1])) for i in 1:(size(master_states, 2)-1)]

# ╔═╡ ee662453-4f43-4788-a5be-6d2ea7ff545e
println("Mean Koopman prediction error: ", mean(errors))

# ╔═╡ d6074c6d-f01f-4080-918f-7d601f3fec0c
# Train-test split
train_ratio = 0.8

# ╔═╡ 8d520fa8-661b-46fc-9ffb-7dd42c12763b
train_size = Int(round(train_ratio * size(X, 2)))

# ╔═╡ 70d92423-5487-471c-afc9-1e68d470cd78
test_size = size(sol)[2] - train_size

# ╔═╡ 3dd60898-c469-4fd9-a0ea-b8ca4df1baff
x_train = X[:, 1:train_size]

# ╔═╡ c99a492f-69da-4651-9db2-ecff40221725
y_train = Y[:, 1:train_size]

# ╔═╡ 4cc593f5-2b82-46e9-b046-6079895b9dd7
x_test = X[:, train_size+1:end]

# ╔═╡ 7e0a118c-eb41-4885-82df-0719f5aa32a8
y_test = Y[:, train_size+1:end]

# ╔═╡ bf55a270-2447-4479-9706-73a647f00b4a
...
# ╔═╡ 9386c848-d797-4c2d-88b1-d881401768b5
loss(x, y) = Flux.Losses.mse(koopman_nn(x), y)

# ╔═╡ b738176c-4592-427f-a3c2-da42af816d16
increment_t = 0.01

# ╔═╡ 76adf188-f8c3-44bf-96ae-046be3058b5d
opt = Flux.setup(Adam(), koopman_nn)

# ╔═╡ b200b039-5841-4f1f-96c9-a938e71cc956
# Train the model
epochs = 2000

# ╔═╡ 351279ef-e856-46fb-9a7e-e38fdaeff0f0
...
# ╔═╡ b187952a-bc0c-47b6-9c27-52e23dac5e84
# Training loop
for epoch in 1:epochs
    grads = Flux.gradient(koopman_nn) do m
        Flux.Losses.mse(K * m(x_train), m(y_train))
    end
    Flux.Optimisers.update!(opt, koopman_nn, grads)
    println("Epoch $epoch | Loss = $(Flux.Losses.mse(K * koopman_nn(x_train), koopman_nn(y_train)))")
end

# ╔═╡ 25a6185e-adad-4de9-9e06-2e8659b01f45
# Compute eigenvalues
eigvals_K = eigvals(K)

# ╔═╡ 08a45702-aa89-4ed0-a7a1-516a49328f39
function koopman_spectrum_fig(eigvals_K)
    fig = Figure()
    ax  = Axis(fig[1,1])
    CairoMakie.scatter!(ax, real.(eigvals_K), imag.(eigvals_K))
    fig
end

# ╔═╡ 20012f74-5875-4175-a8b7-f0f227df8a2a
koopman_spectrum_fig(eigvals_K)

# ╔═╡ bb76a4ab-fbf6-467b-840b-9c89dc722da1
begin
	Plots.scatter(real.(eigvals_K), imag.(eigvals_K),
	...
	plt_koop=Plots.plot!(circle_x, circle_y, lw=2, lc=:red, label="|λ|=1")
	#savefig("E:\\New folder\\Paper to Journal details\\3. IJBC\\Manuscript\\fig_Koopman_spectrum.png")
end

# ╔═╡ 1ecda216-771d-496b-8dfd-cb240b86224e
train_loss = loss(x_train, y_train)

# ╔═╡ 75d779c8-cbfa-4968-bb71-8869b84efc8e
test_loss = loss(x_test, y_test)

# ╔═╡ 75a0d5c4-5e1c-4de2-99c2-f60cb515a4fe
...
# ╔═╡ e5fd80a0-308a-4b71-9d20-d38130d48711
prob_koop = ODEProblem(adjust_control_with_koopman!, u0, tspan, p)

# ╔═╡ c3a3b169-4971-4078-8467-07601aa738bc
sol_koop = solve(prob_koop, Tsit5(), dt = 0.001, adaptive = false)

# ╔═╡ f36d7c4d-189c-4df0-b512-e814301a8b9f
master_koop = [s[1:3] for s in sol_koop.u]

# ╔═╡ ff20c4b5-4c05-47d4-b27b-5032e3c71806
slave_koop = [s[4:6] for s in sol_koop.u]

# ╔═╡ 88592c79-d7d8-4439-bb04-17f5e32fd06e
sync_error_koop = [norm(master_koop[i] - slave_koop[i]) for i in 1:length(master_koop)]

# ╔═╡ 841ab44c-c5d4-404b-8d93-2cebbe29b99e
Plots.plot(sol_koop.t, hcat(master_koop...)', xlabel = "Time (s)", ylabel="System State", label=["xm" "ym" "zm"], title="Master System")

# ╔═╡ b460415a-1dd3-48f6-9c4d-f120acff026d
Plots.plot(sol_koop.t, hcat(slave_koop...)', xlabel = "Time (s)", ylabel="System State", label=["xs" "ys" "zs"], title="Slave System", linestyle=:dash)

# ╔═╡ f2c0aa95-ce44-4cae-8d04-59afa1950063
master_mat = reduce(vcat, master_koop')

# ╔═╡ 8a687de8-c944-4c70-b40e-1db56f826c36
slave_mat = reduce(vcat, slave_koop')

# ╔═╡ f1088020-0820-43bd-a48b-bd396a6b8f13
minval = min(minimum(master_mat), minimum(slave_mat))

# ╔═╡ 983428ac-28d0-4e37-8964-2bbb4e6b5cab
maxval = max(maximum(master_mat), maximum(slave_mat))

# ╔═╡ 6fe4f33b-31ff-4012-9607-b1cc693bc28f
Plots.plot(Plots.heatmap(master_mat, title="Master", clim=(minval, maxval), colorbar=false), Plots.heatmap(slave_mat, title="Slave", clim=(minval, maxval)),)

# ╔═╡ f610c727-c2ab-4fdf-a88f-61f1f26a06d1
diff_mat = abs.(master_mat - slave_mat)

# ╔═╡ 57863afc-3638-42b9-9f19-45e51eb3bb7d
Plots.heatmap(diff_mat, title="|Master - Slave|", xlabel="Variables", ylabel="Time", colorbar_title="|Δ|")

# ╔═╡ b9745356-72e5-494d-b1e4-d5d97d86ac33
Plots.plot(sync_error_koop, yscale=:log10, title="Koopman–Neural Synchronization Error", xlabel = "Time Step", ylabel = "‖Master - Slave‖", label = "Error(t)", legend = :topright)

# ╔═╡ 67969da8-1917-4a18-a768-a2d94bbcf43e
println("Mean synchronization error: ", mean(sync_error_koop))

# ╔═╡ a912d770-1346-4054-ba5e-b31f49b3809f
println("Final error: ", sync_error_koop[end])

# ╔═╡ 1602b0d8-5c4a-4673-ba07-c37d5e6c1c20
begin
    # latent dimension MUST come from architecture, not K_param
    latent_dim = 10   # ← use the SAME value as your encoder output

    # Koopman matrix parameter (plain array, trainable)
    K_param = 0.01 .* randn(latent_dim, latent_dim)
end

# ╔═╡ 4ef70585-96ba-4c5e-a405-019e9713f4ac
loss_joint(x, y) = Flux.Losses.mse(K_param * koopman_nn(x), koopman_nn(y))

# ╔═╡ df38ab0b-1ce1-433e-9912-cf4a98ea35de
struct NeuralKoopman
    koopman_nn
    K
end

# ╔═╡ e0f40fa2-5f0b-4b38-9379-8822b3af9726
model = NeuralKoopman(koopman_nn, K_param)

# ╔═╡ 16b8790f-6395-44db-b224-c0c7611445f6
#opt_joint = Flux.setup(Adam(1e-3), Flux.params(koopman_nn, K_param))
opt_joint = Flux.setup(Adam(1e-3), model)

# ╔═╡ 4130b622-dd52-4fb5-bcb1-655b049cec34
# Synchronization error vs time (log scale to show convergence)
Plots.plot(sol_koop.t, sync_error_koop,
     yscale = :log10,
     linewidth = 2,
     linecolor = :blue,
...
# ╔═╡ b254832c-2674-46bb-a543-5b12080e9f0f
# Annotate the final value
Plots.annotate!([(maximum(sol_koop.t)*0.7, minimum(sync_error_koop)*1.5,
           Plots.text("Error → $(round(sync_error_koop[end], sigdigits=3))", :black, 10))])

# ╔═╡ 8e9e153f-fde6-43d0-aa2f-fd25b86f85a8
...
# ╔═╡ a496ac96-d84f-4298-b98e-5ad7d13c731e
begin
...
# ╔═╡ f75ac73e-d815-43e8-b2b6-7b900a9a9b44
begin
	# Plot all on log scale
	Plots.plot(sol_koop.t, ex, yscale=:log10, lw=2, lc=:red, label="|ex|")
	Plots.plot!(sol_koop.t, ey, lw=2, lc=:green, label="|ey|")
	Plots.plot!(sol_koop.t, ez, lw=2, lc=:blue, label="|ez|",
...
# ╔═╡ cdb5244c-3955-473c-bd85-caa02d9a458c
eigvals_K1 = eigvals(K)

# ╔═╡ ee214d66-eadb-4d8f-87ad-64a4e1c26d68
abs_vals = abs.(eigvals_K1)

# ╔═╡ 4d62ab4e-b6cc-4344-b6d3-456f39b6654a
θ1 = range(0, 2π, length=300)

# ╔═╡ a7d16351-360c-43ef-aded-8be7cc91a370
circle_x1, circle_y1 = cos.(θ1), sin.(θ1)

# ╔═╡ 51a330e1-91c2-4cf8-870c-cbfc3fe48f9d
begin
	Plots.scatter(real.(eigvals_K1), imag.(eigvals_K1),
        marker_z = abs_vals, c = :viridis, ms = 8,
        xlabel="Re(λ)", ylabel="Im(λ)",
        title="Koopman Spectrum (Colored by |λ|)",
        aspect_ratio=1, legend=false)
	Plots.plot!(circle_x1, circle_y1, lc=:red, lw=2, label="|λ|=1")
end

# ╔═╡ 2cc4aa64-4641-4955-ae1d-1aa489b8cb4b
function simulate_sync(params)
    # Example: params = [kp, kd, learning_rate]
    kp, kd, lr = params
    # === Compute synchronization error ===
    xm, ym, zm = sol[1, :], sol[2, :], sol[3, :]
    xs, ys, zs = sol[4, :], sol[5, :], sol[6, :]
    err = mean(sqrt.((xm - xs).^2 .+ (ym - ys).^2 .+ (zm - zs).^2))
    return err  # scalar output for sensitivity analysis
end

# ╔═╡ 8337bba8-6115-45dc-a623-b02d0ae99aba
n = 500 #n = 600000

# ╔═╡ d7071f53-22f2-4563-bfb9-057b1c8743d8
lb = [0.1, 0.1, 0.001]   # lower bounds

# ╔═╡ a1bf8013-8b96-427a-8564-261052e69e36
ub = [2.0, 2.0, 0.01]    # upper bounds

# ╔═╡ baf73909-5cd8-44d4-959e-b5ff7a5558e5
sampler = SobolSample()

# ╔═╡ 96cdff9d-ef76-44d9-80e2-134d52a74a04
A, B = QuasiMonteCarlo.generate_design_matrices(n, lb, ub, sampler)

# ╔═╡ 0971f7f0-bdea-47c6-83c4-43b66b463c27
#f1 = function (p) 
#	prob1 = remake(prob; p = p) 
#	sol1 = solve(prob1, Tsit5(); saveat = dt) 
#	return [mean(sol1[1, :]), maximum(sol1[2, :])] 
#end

# ╔═╡ ab15b4fb-2579-433a-8e06-47b706b72ea7
begin
    # --- Single-sample Sobol QoI ---
    function f1_sobol(p_sobol::Vector{Float64})
        a, b, c = p_sobol

        # fixed control gains (example)
        k1, k2, k3 = -0.5, -0.5, -0.5

        p_full = [a, b, c, k1, k2, k3]

        prob1 = remake(prob; p = p_full)
        sol1  = solve(prob1, Tsit5(); saveat=dt)

        # scalar Quantity of Interest (QoI)
        return mean(sol1[1, :])
    end

    # --- Batch wrapper (REQUIRED for batch=true) ---
    function f1_sobol(P::Matrix{Float64})
        return [f1_sobol(P[:, i]) for i in 1:size(P, 2)]
    end
end

# ╔═╡ fc485493-002d-4312-83ac-31d0044ec0e3
sobol_saltelli = gsa(f1_sobol, Sobol(), A, B; batch=true)

# ╔═╡ 4a725bbf-3b71-42b5-88b0-99823add69f5
begin
    k1 = -0.5
    k2 = -0.5
    k3 = -0.5
...
# ╔═╡ 137169f9-c50b-464a-8029-962972cf64d0
f1 = function (p_sobol)
    a, b, c = p_sobol
    p_full = [a, b, c, k1, k2, k3]
    ...
    return mean(sol1[1, :])   # scalar QoI
end

# ╔═╡ 34488d7e-8eb3-413b-8131-369316527cbc
begin
    _rng = StableRNG(1234)
    ...
    param_names = ["a","b","c","k1","k2","k3"]
    param_idx = 1:length(morris_sens.means_star[1, :])

    # Master
    p11 = Plots.scatter(
        param_idx,
        morris_sens.means_star[1, :],
        xticks = (param_idx, param_names),
    ...
    # Predator / Slave (same single-output Morris result)
    p12 = Plots.scatter(
        param_idx,
        morris_sens.means_star[1, :],   # ← FIXED LINE
        xticks = (param_idx, param_names),
        xlabel = "Parameter index",
        ylabel = "μ*",
        title  = "Predator",
    ...
    Plots.plot(p11, p12, layout = (1,2), size = (800, 400))
end


# ╔═╡ de4f208d-f244-4872-9401-65df83629039
begin
	function simulate_sync_batch(P::Matrix)
    	Y = zeros(1, size(P, 2))   # 1 QoI × N samples
    	for i in 1:size(P, 2)
        	Y[:, i] .= simulate_sync(P[:, i])
    	end
    	return Y
	end
...
# ╔═╡ 8d9a13c8-08eb-4a68-8fb5-abe8510d5a56
println("First-order indices: ", res.S1)

# ╔═╡ ac47c2ad-2329-493e-8045-3067f872d55d
println("Total-order indices: ", res.ST)

# ╔═╡ 5e70c75c-1131-4489-b718-4122b5363e86
bar(res.S1, legend=false, title="First-order Sobol Sensitivity", xlabel="Parameters", ylabel="Sensitivity Index")

# ╔═╡ 38edaca1-854c-4532-9498-e80cc28d502a
begin
    y = res.S1[1, :]          # ← THIS IS THE KEY FIX
    x = 1:length(y)
...
# ╔═╡ cf711752-3ef8-4ff9-9e30-01d9856b8310
begin
	test_loss_hist = Float64[]
	train_loss_hist = Float64[]


	push!(test_loss_hist, test_loss)
	push!(train_loss_hist, train_loss)

    epochs1 = 1:length(train_loss_hist)
    ...
    Plots.plot!(
        epochs1,
        test_loss_hist,          # must be a VECTOR
        lw = 2,
        label = "Testing Loss"
    )
    ...
    Plots.xlabel!("Epochs")
    Plots.ylabel!("Loss")
    Plots.title!("Training and Testing Loss")
end

# ╔═╡ aec7bafd-aacb-4831-a4fe-d478680394e0
Plots.plot(sol_koop.t, sync_error_koop,
	ylabel="Synchronization Error",
	xlabel="Time",
	title="Mean Synchronization Error Over Time",
	lw=2, label="||x_{m}(t) - x_{s}(t)||")

# ╔═╡ 655149b3-98b0-4300-ba44-1f0f157dcf2a
begin
	window = 2800 # last 100 points
	Plots.plot(sol_koop.t[end-window+1:end], sync_error_koop[end-window+1:end],
		ylabel="Final Error",
		xlabel="Time",
		title="Final Steady-State Error",
		lw=2, label="e(t)")
	#savefig("E:\\New folder\\Paper to Journal details\\3. IJBC\\Manuscript\\Final_error.png")
end

# ╔═╡ 02009d95-0f6b-4f39-8745-f65dc8197b00
begin
	Plots.plot(sol_koop.t, sync_error_koop, label="Nominal", lw=2)
	#Plots.plot!(sol_noisy.t, sync_error_noisy, label="With Measurement Noise", lw=2)
	Plots.xlabel!("Time")
	Plots.ylabel!("Synchronization Error")
	Plots.title!("Robustness of Koopman-Neural Synchronization")
end

# ╔═╡ 102061bf-9999-4f59-8b96-ad5d08c50c1c
bar(
        res.S1[1, :],                          # ← extract vector
        xticks = (1:4, ["a", "b", "c", "k_gain"]),
        xlabel = "Parameter",
        ylabel = "First-Order Sobol Index",
        title  = "Sensitivity of Synchronization to Parameter Variations",
        legend = false
    )

# ╔═╡ 5aa86401-0b8c-41d1-9021-59ad864580bf
t = (0:size(X,2)-1) .* dt

# ╔═╡ bd7eb709-a856-40e7-99be-f30fb57b7443
begin
	p1 = Plots.plot(t, X[1,:], label="Master x₁", lw=2)
	Plots.plot!(p1, t, Y[1,:], label="Slave x₁", lw=2, ls=:dash)
	
	p2 = Plots.plot(t, X[2,:], label="Master x₂", lw=2)
	Plots.plot!(p2, t, Y[2,:], label="Slave x₂", lw=2, ls=:dash)
	
	p3 = Plots.plot(t, X[3,:], label="Master x₃", lw=2)
	Plots.plot!(p3, t, Y[3,:], label="Slave x₃", lw=2, ls=:dash)
	
	Plots.plot(p1, p2, p3, layout=(3,1), size=(900,700))
end

# ╔═╡ 303e19dd-aa8b-44d8-9075-e226699676af
begin
	e = Y .- X
	e_norm = vec(norm.(eachcol(e)))
...
# ╔═╡ a0bfdb6d-4b6e-46a1-bcfa-5b30ee904ee5
res.S1

# ╔═╡ 65d33fd6-33a1-4240-839d-1ee4717275e5
any(isfinite, res.S1)

# ╔═╡ c5027c7b-20cf-4d2c-8520-bfab3b6042b3
begin
    param_names1 = ["a", "b", "c", "k₁", "k₂", "k₃"]
    μstar = morris_sens.means_star[1, :]

    p21 = Plots.scatter(
        1:length(μstar),
        μstar,
        xticks = (1:length(param_names1), param_names1),
        xlabel = "Model parameters",
        ylabel = "Morris μ*",
        title  = "Morris Sensitivity Analysis (Synchronization Error)",
        markersize = 8,
        legend = false
    )
	...
	# Save for manuscript (high resolution)
    savefig(p21, "Fig_Morris_Sensitivity.png")
...
manifest_format = "2.0"
project_hash = "40d66bdc4b8c2a2690053c6af383f6e54c82927e"
...
git-tree-sha1 = "a7d7d6194b3582455be7dd9495561626b3803cc5"
...
git-tree-sha1 = "df895f2a7fc136d2719eb6934f66d964b2a594af"
...
[[deps.Copulas]]
deps = ["BigCombinatorics", "Combinatorics", "Distributions", "ForwardDiff", "HCubature", "LambertW", "LinearAlgebra", "LogExpFunctions", "MvNormalCDF", "Optim", "PolyLog", "Printf", "QuadGK", "Random", "Roots", "SpecialFunctions", "Statistics", "StatsBase", "StatsFuns", "TaylorSeries"]
git-tree-sha1 = "ba47eec0ae3968b311bb1fa61736005e7c9829c8"
...
[[deps.ExcelFiles]]
deps = ["DataValues", "Dates", "ExcelReaders", "FileIO", "IterableTables", "IteratorInterfaceExtensions", "Printf", "TableShowUtils", "TableTraits", "TableTraitsUtils", "Test"]
git-tree-sha1 = "fce901cb058dd480cdd204837390403ab4b5f150"
...
deps = ["Combinatorics", "ComplexityMeasures", "Copulas", "Distributions", "FFTW", "ForwardDiff", "KernelDensity", "LinearAlgebra", "Parameters", "QuasiMonteCarlo", "Random", "RecursiveArrayTools", "Statistics", "StatsBase", "ThreadsX", "Trapz"]
git-tree-sha1 = "e9f673523e5b6f731010ea4ba93d0d7f9db78ed6"
...
[[deps.LambertW]]
deps = ["IrrationalConstants"]
git-tree-sha1 = "c6cd4fe119e21ebb1ca8adfaf32c989465c8d920"
...
git-tree-sha1 = "e416c7f4f02defc960b08b4783e6cce673021384"
